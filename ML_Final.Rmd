...
For match this goal, I used ensemble model of Generalized Boosted Regression Model, Random Forest, Linear Discriminant Analysis. And for reducing dimension to increase training speed, PCA is used. After all this approach, I could get 0.98% of accuracy.
...
I made plan to use ensemble model with stacking gbm, rf, lda. And PCA can reduce the size of variables for training performance.
...
```{r lda}
set.seed(1234)
fit_lda <- train(classe ~ ., data = data.trainPca, method = "lda")
pred_lda <- predict(fit_lda, newdata = data.validPca)
confusionMatrix(pred_lda,data.validPca$classe)
```

```{r ensemble}
predDF <- data.frame(pred_gbm, pred_rf, pred_lda, classe = data.validPca$classe)
...
The model what has best performance is Random forest as you see. And the ensemble model of gbm, rf, lda gets better only 0.0028% of accuracy.
...
pred_lda_test <- predict(fit_lda, newdata = data.testPCA)

predDF_test <- data.frame(pred_gbm = pred_gbm_test, pred_rf = pred_rf_test, pred_lda = pred_lda_test)
