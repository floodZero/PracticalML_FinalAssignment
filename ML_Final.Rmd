...
I made plan to use ensemble model with stacking gbm, rf, lda. There's no need for PCA transform when use random forest, but I'll use it for other models.
...
```{r glm}
fit_glm <- train(classe ~ ., data = data.trainPca, method = "glm", trControl = trainControl(method="repeatedcv", number=10, repeats=3))
pred_glm <- predict(fit_glm, data.validPca)
confusionMatrix(pred_glm, data.validPca$classe)$overall[1]
accuracy(pred_glm, data.validPca$classe)
```


```{r gbm}
set.seed(1234)
fit_gbm <- train(classe ~ ., data = data.trainPca, method = "gbm", trControl = trainControl(method="cv", number=10))
...
```{r rf}
set.seed(1234)
fit_rf <- train(classe ~ ., data = data.trainPca, method = "rf", trControl = trainControl(method="cv", number=10))
